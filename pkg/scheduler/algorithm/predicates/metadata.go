...
type predicateMetadata struct {
	...
	antiAffinityPodsToTopologyPair map[*v1.Pod][]topologyPair
...
func (pfactory *PredicateMetadataFactory) GetMetadata(pod *v1.Pod, nodeNameToInfoMap map[string]*schedulercache.NodeInfo) algorithm.PredicateMetadata {
	...
	podsToTopologyPairs, topologyPairToPods, err := getMatchingTopologyPairs(pod, nodeNameToInfoMap)
	if err != nil {
		return nil
	}
	...
	predicateMetadata := &predicateMetadata{
		...
		antiAffinityPodsToTopologyPair:     podsToTopologyPairs,
	}
...
func (meta *predicateMetadata) RemovePod(deletedPod *v1.Pod) error {
	...
	for _, pair := range meta.antiAffinityPodsToTopologyPair[deletedPod] {
...
func (meta *predicateMetadata) AddPod(addedPod *v1.Pod, nodeInfo *schedulercache.NodeInfo) error {
	...
	matchingPodToTopologyPairs, podTopologyPairsToMatchingPods, err := getMatchingTopologyPairsOfExistingPod(meta.pod, addedPod, nodeInfo.Node())
	if err != nil {
		return err
	}
	if len(matchingPodToTopologyPairs) > 0 {
		for pair, pods := range podTopologyPairsToMatchingPods {
		...
		for pod, pairs := range matchingPodToTopologyPairs {
			meta.antiAffinityPodsToTopologyPair[pod] = append(meta.antiAffinityPodsToTopologyPair[pod], pairs...)
		}
	}
...
func (meta *predicateMetadata) ShallowCopy() algorithm.PredicateMetadata {
	...
	newPredMeta.antiAffinityPodsToTopologyPair = make(map[*v1.Pod][]topologyPair)
	for k, v := range meta.antiAffinityPodsToTopologyPair {
		newPredMeta.antiAffinityPodsToTopologyPair[k] = append([]topologyPair(nil), v...)
	}
