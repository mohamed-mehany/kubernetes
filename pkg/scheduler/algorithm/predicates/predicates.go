...
func getMatchingTopologyPairs(pod *v1.Pod, nodeInfoMap map[string]*schedulercache.NodeInfo) (*topologyPairsMaps, error) {
	...
	topologyMaps := &topologyPairsMaps{topologyPairToAntiAffinityPods: make(map[topologyPair][]*v1.Pod),
	...
	processNode := func(i int) {
		...
		nodeTopologyMaps := &topologyPairsMaps{topologyPairToAntiAffinityPods: make(map[topologyPair][]*v1.Pod),
			antiAffinityPodToTopologyPairs: make(map[string][]topologyPair)}
		for _, existingPod := range nodeInfo.PodsWithAffinity() {
			...
			if len(nodeTopologyMaps.antiAffinityPodToTopologyPairs) > 0 {
...
func (c *PodAffinityChecker) satisfiesExistingPodsAntiAffinity(pod *v1.Pod, meta algorithm.PredicateMetadata, nodeInfo *schedulercache.NodeInfo) (algorithm.PredicateFailureReason, error) {
	...
	for topologyKey, topologyValue := range node.Labels {
		if violatedPods, ok := topologyMaps.topologyPairToAntiAffinityPods[topologyPair{key: topologyKey, value: topologyValue}]; ok {
